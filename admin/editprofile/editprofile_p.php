...
if(checkUserData($userid,$conpassword)) {
	updateTuser($userid,$email,$tel,$name,$role);
	$str.=("<register>");
	$str.=("<result>Y</result>");
...
function updateTuser($userid,$email,$tel,$name,$role) {
	$updateSQL = "update tuser set email='$email', tel='$tel', name='$name',usergrpid='$role' where userid='$userid'";
